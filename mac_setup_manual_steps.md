...
- [ ] Copy relevant Claude Code project memories from the previous machine.
  - Each project's memories live at `~/.claude/projects/<project>/memory/` (e.g. `~/.claude/projects/-Users-cbeach-development-dotfiles/memory/`). Copy the `memory/` subfolder for each project you want to preserve onto the new machine at the same path.
...
## Claude Code
- [ ] Populate `~/.rollbar-mcp.json` with access tokens.
  - [Instructions](https://github.com/rollbar/rollbar-mcp-server?tab=readme-ov-file#configuration)  
  - Existing access tokens are stored in your password manager.
